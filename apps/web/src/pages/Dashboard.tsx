...
export const Dashboard: React.FC = () => {
  ...
  const toneClasses = {
    ...
    success: 'bg-[#0acf97] text-white',
    warning: 'bg-[#ffbc00] text-slate-950',
    danger: 'bg-[#fa5c7c] text-white',
  } as const;
      ...
      <section className="grid gap-6 xl:grid-cols-[1.35fr_1fr]">
        <div className="panel-card p-6">
          <div>
            ...
            <h1 className="mt-3 max-w-2xl text-3xl font-extrabold leading-tight text-slate-700 md:text-[34px]">
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-3">
              <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
                <div className="flex items-center justify-between">
                  ...
                  <Shield className="h-4 w-4 text-[#727cf5]" />
                </div>
              ...
              <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
                <div className="flex items-center justify-between">
                  ...
                  <Clock3 className="h-4 w-4 text-[#ffbc00]" />
                </div>
                <p className="mt-3 text-3xl font-extrabold text-slate-700">
                  {dashboard.overdueTasks}
                </p>
                <p className="mt-2 text-sm text-slate-400">Acciones que requieren atencion</p>
              </div>
              <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-bold text-slate-500">Alertas abiertas</span>
                  <AlertTriangle className="h-4 w-4 text-[#fa5c7c]" />
                </div>
                <p className="mt-3 text-3xl font-extrabold text-slate-700">{alerts.length}</p>
                <p className="mt-2 text-sm text-slate-400">Calculadas automaticamente</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="panel-card overflow-hidden">
          <div className="bg-[#727cf5] px-6 py-5 text-white">
          ...
          <div className="space-y-4 p-6">
            ...
            <div className="flex items-center justify-between rounded-xl bg-slate-50 px-4 py-3">
              ...
              <Activity className="h-4 w-4 text-[#ffbc00]" />
            </div>
          </div>
        </div>
      </section>
          ...
          <div key={stat.label} className="panel-card p-5">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="mt-2 text-3xl font-extrabold text-slate-700">{stat.value}</p>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.5fr_1fr]">
        <section className="panel-card p-6">
          ...
          <div className="space-y-4">
            {dashboard.recentActivity.map((activity) => (
              ...
              >
                <div className="rounded-xl bg-white p-3 text-[#727cf5] shadow-sm">
                  <Activity className="h-5 w-5" />
                </div>
                ...
                <span className="text-xs font-medium text-slate-400">{activity.timestamp}</span>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="panel-card p-6">
          <div className="flex items-center justify-between">
            ...
            <AlertTriangle className="h-5 w-5 text-[#ffbc00]" />
          </div>

          <div className="mt-5 space-y-3">
            {alerts.slice(0, 4).map((alert) => (
              <div key={alert.id} className="rounded-xl border border-slate-100 bg-slate-50 p-4">
