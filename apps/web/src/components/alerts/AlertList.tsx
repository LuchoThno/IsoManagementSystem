...
export const AlertList: React.FC<AlertListProps> = ({ alerts }) => {
  return (
    <div className="space-y-4">
      {alerts.map(alert => (
        <div key={alert.id} className={`rounded-xl border p-4 shadow-panel ${getPriorityColor(alert.priority)}`}>
          <div className="flex items-start justify-between gap-4">
            <div>
              <h3 className="text-base font-extrabold text-app-text">{alert.title}</h3>
